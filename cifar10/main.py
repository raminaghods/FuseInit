...
fuses multiple layers from last layer of the network one at a time

this code needs a GPU node
"""
...
for tr in range(0,num_trials):
    ...
    [Lc_a0,Lc_a1,Ld] = Create_rnd_Net(L0,x_train.shape[1:],num_classes,n_clayers,weight_decay,True,0,0,0,0)
    ...
    Wc = []
    bc = []
    Wc.append( model_2rnd.get_layer('conv1').get_weights()[0])
    bc.append( model_2rnd.get_layer('conv1').get_weights()[1])
    Wc.append(model_2rnd.get_layer('conv2').get_weights()[0])
    bc.append(model_2rnd.get_layer('conv2').get_weights()[1])
    ...
    for f in range(0,num_Fuse):
        ...
        [Lc_a0_Bsg,Lc_a1_Bsg,Ld_Bsg] = Create_rnd_Net(L0_1Bsg,x_train.shape[1:],num_classes,n_clayers-f-1,weight_decay,False,
                            np.transpose(w_init),np.squeeze(b_init),Wc,bc)
        ...
        [tmp,tmp,Ld_1rnd] = Create_rnd_Net(L0_1rnd,x_train.shape[1:],num_classes,n_clayers-f-1,weight_decay,True,0,0,0,0)
...
#+'_Kernel-len'+str(K[0])+'-'+str(K[1])+'-'+str(K[2])+'-'+str(K[3])
